...
async def  processWall(request):
    wall = BlockWall(data= await request.json())
    ...
    finally: del(payload); del(wall)

async def getBuildingRates(request):
    wall = BlockWall()
    return JSONResponse(await wall.building_rates)
